...
local function format_entry(entry) return string.format("%-25s [%s]", entry.task, entry.group) end
...
local function run_task_action(prompt_bufnr)
  ...
  if selection and selection.value then terminal.run_task(selection.value.task) end
end
...
local function pick_tasks_filtered_by_group(tasks, group)
  ...
  local filtered = {}
  for _, t in ipairs(tasks) do
    if t.group == group then table.insert(filtered, t) end
  end
  if #filtered == 0 then
