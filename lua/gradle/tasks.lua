...
local function sort_tasks(tasks)
  table.sort(tasks, function(a, b)
    if a.group == b.group then return a.task < b.task end
...
function M.load_tasks(refresh)
  if not refresh and M.cached_tasks then return M.cached_tasks, M.cached_status end
...
function M.load_tasks_async(force_refresh, cb)
  ...
  if not force_refresh and M.cached_tasks then
    if cb then cb(M.cached_tasks, M.cached_status) end
    return
  end
  ...
  local function handle_result(code, stdout, stderr)
    ...
    if status == "BUILD FAILED" then
      vim.schedule(
        function() vim.notify("[gradle.nvim] Gradle build failed while listing tasks", vim.log.levels.ERROR) end
      )
      M.cached_tasks = {}
      ...
      vim.schedule(function()
        ...
        if status then msg = msg .. " - " .. status end
        if code ~= 0 then msg = msg .. " (exit " .. tostring(code) .. ")" end
        vim.notify(msg)
      end)
    end

    M._loading = false
    if cb then vim.schedule(function() cb(M.cached_tasks, M.cached_status) end) end
  end
  ...
  if has_vim_system then
    ...
    vim.system(
      argv,
      { text = true },
      function(obj) handle_result(obj.code or 0, obj.stdout or "", obj.stderr or "") end
    )
  else
    -- Neovim <=0.9: jobstart
    ...
    local job_id = vim.fn.jobstart(argv, {
      ...
      on_stdout = function(_, data)
        if data and #data > 0 then table.insert(stdout_chunks, table.concat(data, "\n")) end
      end,
      on_stderr = function(_, data)
        if data and #data > 0 then table.insert(stderr_chunks, table.concat(data, "\n")) end
      end,
